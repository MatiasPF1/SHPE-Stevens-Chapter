...
const PartnershipTiers = () => {
  return (
    <section className="py-6 -translate-y-14" style={{ backgroundColor: "var(--color-page-bg)" }}>
      <div className="max-w-[1500px] mx-auto px-5 text-center">


        <h2 
          className="text-4xl font-extrabold mb-20"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-8 items-stretch">
          {partnershipTiers.map((tier) => (
            <div key={tier.name} className={`p-8 rounded-lg shadow-lg ${tier.bgColor} ${tier.textColor} flex flex-col`}>
              ...
              <a href="mailto:shpe.sit@gmail.com" className={`mt-auto py-2 px-4 rounded-lg font-bold ${tier.buttonColor} ${tier.buttonTextColor} self-center`}>
                Contact Us
              </a>
            </div>
          ))}
